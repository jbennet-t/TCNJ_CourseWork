...
// Define w0 as 2PI/100 = PI/50

// cos(w0) ~ 1.0
// sin(w0) ~ w0, small angle approximation
#define COS_0   (FIXED_ONE)
#define SIN_0   ( (int) (FIXED_ONE * 3.1415926535897 / 50 + 0.5))

// Oscillator state variables
int64_t     x_real=FIXED_ONE;   // Initial value of x (complex number) is 1+j0, in fixed point
int64_t     x_imag=0;           // Initial value of x (complex number) is 1+j0, in fixed point
...
CY_ISR(isr_1_Interrupt)
{
    ...
    int64_t     rsq;                // Holds square of real component
    int64_t     isq;                // Holds square of imag component

    int32_t     sgn_real;           // +/-1, based on x_real > 0 or < 0
    int32_t     sgn_imag;           // +/-1, based on x_imag > 0 or < 0
    
    x_real = x_real*COS_0 - x_imag*SIN_0;                           // QM.N * QM.N -> Q2M.2N
    ...
    build_plot_str( msg_str, (int) x_real, (int) x_imag );      // Builds the string for output (at 57,600 baud, to serial port
    UART_PutString(msg_str);                                    // Sends string to serial port
