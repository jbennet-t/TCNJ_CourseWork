...
CY_ISR(isr_2_Interrupt)
{
    ...
    /* `#START isr_2_Interrupt` */
        // x_real = x_real*COS_0 - x_imag*SIN_0
        // x_imag = x_real*SIN_0 + x_imag*COS_0
    ...
    rsq = x_real*x_real;                            // QM.N * QM.N -> Q2M.2N
    rsq = (rsq + FIXED_ONE_HALF) >> FIXED_N;        // convert back to QM.N, with rounding
    
    isq = x_imag*x_imag;                            // QM.N * QM.N -> Q2M.2N
    ...
    VDAC8_1_SetValue((x_real >> (FIXED_N-6))+ 128);
